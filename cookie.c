...
cookie_engine_create(const char *secret, const char *salt, int pbkdf2_rounds)
{
  ...
  if(!PKCS5_PBKDF2_HMAC(secret, strlen(secret),
                        (const uint8_t *)salt, salt ? strlen(salt) : 0, pbkdf2_rounds,
  ...
  EVP_EncryptInit_ex(ce->enc, EVP_aes_192_ccm(), NULL, NULL, NULL);
  EVP_CIPHER_CTX_ctrl(ce->enc, EVP_CTRL_CCM_SET_IVLEN, COOKIE_NONCE_LEN, NULL);
  EVP_CIPHER_CTX_ctrl(ce->enc, EVP_CTRL_CCM_SET_TAG, COOKIE_TAG_LEN, NULL);
  EVP_EncryptInit_ex(ce->enc, NULL, NULL, key, NULL);

  ce->dec = EVP_CIPHER_CTX_new();
  EVP_EncryptInit_ex(ce->dec, EVP_aes_192_ccm(), NULL, NULL, NULL);
  EVP_CIPHER_CTX_ctrl(ce->dec, EVP_CTRL_CCM_SET_IVLEN, COOKIE_NONCE_LEN, NULL);
  EVP_CIPHER_CTX_ctrl(ce->dec, EVP_CTRL_CCM_SET_TAG, COOKIE_TAG_LEN, NULL);
...
cookie_encode(cookie_engine_t *ce, const ntv_t *msg)
{
  ...
  const size_t encrypted_len = COOKIE_NONCE_LEN + COOKIE_TAG_LEN + msg_plaintext.mq_size;
  ...
  EVP_EncryptInit_ex(ce->enc, NULL, NULL, NULL, encrypted);

  int outlen = 0;

  EVP_EncryptUpdate(ce->enc, encrypted + COOKIE_NONCE_LEN + COOKIE_TAG_LEN,
                    &outlen, pt, msg_plaintext.mq_size);

  int tmplen;
  EVP_EncryptFinal_ex(ce->enc, encrypted + COOKIE_NONCE_LEN + COOKIE_TAG_LEN,
                      &tmplen);

  outlen += tmplen;
  EVP_CIPHER_CTX_ctrl(ce->enc, EVP_CTRL_CCM_GET_TAG, COOKIE_TAG_LEN,
                      encrypted + COOKIE_NONCE_LEN);
  pthread_mutex_unlock(&ce->enc_mutex);
  return base64_encode_a(encrypted, encrypted_len, BASE64_URL);
}
...
cookie_decode(cookie_engine_t *ce, const char *str)
{
  ...
  EVP_DecryptInit_ex(ce->dec, NULL, NULL, NULL, bin);

  EVP_CIPHER_CTX_ctrl(ce->dec, EVP_CTRL_CCM_SET_TAG, COOKIE_TAG_LEN,
                      bin + COOKIE_NONCE_LEN);
